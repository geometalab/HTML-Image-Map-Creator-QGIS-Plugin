...
VALID_GEOMETRY_TYPES = {QGis.WKBPolygon, QGis.WKBMultiPolygon, QGis.WKBPoint}
...
class ImageMapPlugin:
  ...
  def writeHtml(self):
    ...
    if isInfoChecked:
        ...
        html.append(u'function displayBox(centerX, centerY, id) { var infoBox = document.getElementById("info-box"); var infoWidthToCenter = infoBox.offsetWidth / 2; var infoHeight = infoBox.offsetHeight; infoBox.innerHTML = infoBoxes[id]; var left = (document.documentElement && document.documentElement.scrollLeft) || document.body.scrollLeft; var top = (document.documentElement && document.documentElement.scrollTop) || document.body.scrollTop; infoBox.style.left = (centerX - infoWidthToCenter - offsetLeft) + left + "px"; infoBox.style.top = (centerY - infoHeight - 15) + top + "px"; } ')
        html.append(u'function hideBox() { document.getElementById("info-box").className = "hidden"; } ')
    if isLabelChecked:
        html.append(u'function displayElementText(centerX, centerY, id) { var titleBoxContainer = document.getElementsByClassName("title-box")[0]; var titleBox = document.createElement("title-box"); titleBox.className = "title-box"; titleBox.innerHTML = labels[id]; titleBoxContainer.appendChild(titleBox); var titleWidthToCenter = titleBox.offsetWidth / 2; var titleHeightToCenter = titleBox.offsetHeight; titleBox.style.left = (centerX - titleWidthToCenter) + "px"; titleBox.style.top = (centerY + titleHeightToCenter + 5) + "px"; } ')
    ...
    if self.labels:
        html.append(u'var labels = ["' + '", "'.join(self.labels) + '"]; ')
    if self.infoBoxes:
        html.append(u'var infoBoxes = ["' + '", "'.join(self.infoBoxes) + '"]; ')
  ...
  def handleGeom(self, feature, selectedFeaturesIds, doCrsTransform, bufferDistance):
    ...
    if geom.wkbType() == QGis.WKBPoint:  # 1 = WKBPoint
        ...
        area_strings = [self.ring2area(feature, ring, projectExtent, projectExtentAsPolygon) for ring in polygon]
        html.append(''.join(area_strings))
  ...
  def ring2area(self, feature, ring, extent, extentAsPoly):
    ...
    if self.imageMapPluginGui.isInfoBoxChecked():
        ...
        self.infoBoxes.append(self.removeNewLine(param))
    htm = htm + 'coords="'
    lastPixel = [0, 0]
    insideExtent = False
    coordCount = 0
